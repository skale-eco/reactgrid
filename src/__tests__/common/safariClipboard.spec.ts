...
import { browserstackURL, getRemoteDesktopSafariCapabilities, getSafariCapabilities } from '../options';
...
describe('Safari clipboard', () => {
    ...
    beforeAll(async () => {
        driver = new Builder()
            ...
            .withCapabilities(getRemoteDesktopSafariCapabilities('Desktop Safari clipboard'))
    ...
    it.skip('multi cell text is pasted into single cell', async () => { // ✅
        await utils.runner(async () => {
            ...
            // can't paste after `writeTextToClipboard`
            await utils.paste();
            ...
            await utils.runAssertion(async () => {
                if (utils.getConfig().isPro) {
                    ...
                    // in desktopSafari pasted text is joined with tab char
    ...
    it('should paste content from other source', async () => { // ✅
        await utils.runner(async () => {
            ...
            await actions
                ...
                .perform();
            if (utils.getTestedBrowser() === 'desktopSafari' || utils.getTestedBrowser() === 'remoteDesktopSafari') {
                await utils.cut();
            } else {
